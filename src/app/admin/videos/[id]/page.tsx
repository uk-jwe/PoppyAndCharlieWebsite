  ...
  async function update(formData: FormData) {
    ...
    await prisma.video.update({
      ...
      data: {
        ...
        order:       parseInt(formData.get('order') as string) || video.order,
      },
    })
    ...
    <div className="max-w-xl">
      ...
      <form action={update} className="space-y-4">
        <div>
          ...
          <input type="text" name="title" required defaultValue={video.title}
            className="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-300" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">YouTube URL *</label>
          <input type="url" name="youtubeUrl" required defaultValue={video.youtubeUrl}
            className="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-300" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="number" name="order" defaultValue={video.order} min={1}
            className="w-24 border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-300" />
        </div>
        <div className="flex items-center gap-2">
          <input type="checkbox" name="featured" id="featured" defaultChecked={video.featured} className="rounded" />
